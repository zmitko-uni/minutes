...
2. Put new logic in **`ts/minutes/`** (or `app/minutes_*.main.ts` for main process).
3. Upstream files: **thin hooks only** — document every touch in **[docs/MINUTES-PATCHES.md](docs/MINUTES-PATCHES.md)**.
...
- Update **user docs** if behavior changes: `images/minutes/prirucka.md`, optionally `CHANGELOG.md` under `[Unreleased]`.
