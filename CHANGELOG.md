...
Před release doplňte sekci **[Unreleased]** (Added / Changed / Fixed). Workflow *Release Minutes* ji při bumpu
verze přesune do `## [x.y.z] - datum` a vygeneruje GitHub Release poznámky. Pokud je [Unreleased] prázdná,
doplní se body z git commitů od posledního tagu.

## [Unreleased]

### Added
- `minutes-quality-gate.bat` — lokální generate + typecheck jako na CI
- Release pipeline automaticky přesouvá [Unreleased] do verzované sekce CHANGELOG

### Changed
- Přejmenování uuMinutes → Minutes (moduly, IPC, ikony, skripty)
- Výchozí Whisper pipeline: celý soubor → align speaker log → volitelná AI korekce
