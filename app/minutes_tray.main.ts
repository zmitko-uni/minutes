...
import { APP_DISPLAY_NAME } from '../ts/minutes/branding.std.ts';
import { isMinutesBrandingEnabled } from './minutes_icon.main.ts';

const TRAY_PREFIX = 'minutes-tray-icon';

export function getMinutesTrayToolTip(): string {
...
export function resolveTrayIconImagePath(
  ...
  const prefix = isMinutesBrandingEnabled(config) ? TRAY_PREFIX : 'signal-tray-icon';
...
export function resolveTrayToolTip(
  ...
  return isMinutesBrandingEnabled(config)
    ? getMinutesTrayToolTip()
    : signalDesktopLabel;
}
