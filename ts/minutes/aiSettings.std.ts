...
export type AiOutputLanguageOption = Readonly<{
  code: string;
  /** Popisek v UI (selectbox). */
  label: string;
  /** Název jazyka pro AI prompt. */
  promptLabel: string;
}>;

export const AI_OUTPUT_LANGUAGE_OPTIONS: ReadonlyArray<AiOutputLanguageOption> =
  [
    { code: 'cs', label: 'Česky', promptLabel: 'čeština' },
    { code: 'en', label: 'Anglicky', promptLabel: 'English' },
    { code: 'sk', label: 'Slovensky', promptLabel: 'slovenčina' },
    { code: 'de', label: 'Německy', promptLabel: 'Deutsch' },
    { code: 'pl', label: 'Polsky', promptLabel: 'polština' },
  ];

export function normalizeAiOutputLanguage(code: string): string {
  const normalized = code.trim().toLowerCase();
  if (!normalized) {
    return DEFAULT_AI_SETTINGS.outputLanguage;
  }
  const known = AI_OUTPUT_LANGUAGE_OPTIONS.find(
    option => option.code === normalized
  );
  return known?.code ?? DEFAULT_AI_SETTINGS.outputLanguage;
}

export function getAiOutputLanguageLabel(code: string): string {
  const normalized = normalizeAiOutputLanguage(code);
  const option = AI_OUTPUT_LANGUAGE_OPTIONS.find(
    item => item.code === normalized
  );
  return option?.promptLabel ?? normalized;
}

export function getAiOutputLanguageUiLabel(code: string): string {
  const normalized = normalizeAiOutputLanguage(code);
  const option = AI_OUTPUT_LANGUAGE_OPTIONS.find(
    item => item.code === normalized
  );
