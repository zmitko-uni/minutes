...
const log = createLogger('uuminutes/callRecorder');

export type UuMinutesWorkletMessage = Readonly<
  | {
      type: 'chunk';
...
async function initContext(): Promise<AudioContext> {
  ...
  await context.audioWorklet.addModule('bundles/workers/uuMinutesMp3Encoder.js');
  return context;
}
...
export class CallRecorder {
  ...
  async start(streams: ReadonlyArray<MediaStream>): Promise<boolean> {
    ...
    const worklet = new AudioWorkletNode(context, 'uuminutes-mp3-encoder', {
      numberOfInputs: 1,
      numberOfOutputs: 0,
      channelCount: 2,
    ...
    worklet.port.onmessage = ({ data }: { data: UuMinutesWorkletMessage }) => {
      if (data.type === 'chunk') {
...
export async function getLoopbackAudioStream(): Promise<MediaStream | null> {
  try {
    const sourceId = await ipcRenderer.invoke(
      'uuminutes:get-loopback-audio-source'
    );
