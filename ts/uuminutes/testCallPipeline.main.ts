...
const log = createLogger('uuminutes/testCallPipeline');
...
export async function runCallPipelineTest(options: {
  ...
  const recordingBase =
    ...
    process.env.UUMINUTES_TEST_RECORDING ??
  ...
  if (result.summaryPath) {
    ...
    if (!settings.aiEnabled) {
      ...
      log.warn(
        'summary not generated: nelze nacist API klic (set UUMINUTES_AI_API_KEY nebo spustte Minutes aulohu jednou)'
      );
    } else {
