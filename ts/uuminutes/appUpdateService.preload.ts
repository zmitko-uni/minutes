...
const log = createLogger('uuminutes/appUpdateService');
...
export async function checkForAppUpdate(): Promise<AppUpdateCheckResult> {
  return ipcRenderer.invoke('uuminutes:check-for-app-update');
}
...
export async function getStartupAppUpdateState(): Promise<{
  ...
  return ipcRenderer.invoke('uuminutes:get-startup-app-update-state');
}

export async function getPendingAppUpdate(): Promise<PendingAppUpdate | null> {
  return ipcRenderer.invoke('uuminutes:get-pending-app-update');
}
...
export async function downloadAppUpdate(options: {
  ...
  return ipcRenderer.invoke('uuminutes:download-app-update', options);
}

export async function installPendingAppUpdate(options: {
  version?: string;
} = {}): Promise<{ installerPath: string }> {
  return ipcRenderer.invoke('uuminutes:install-pending-app-update', options);
}
...
export async function downloadAndInstallAppUpdate(options: {
  ...
  return ipcRenderer.invoke('uuminutes:download-and-install-app-update', options);
}
...
export function subscribeAppUpdateProgress(
  ...
  ipcRenderer.on('uuminutes:app-update-progress', handler);
  return () => {
    ipcRenderer.removeListener('uuminutes:app-update-progress', handler);
  };
}
