...
import {
  UUMINUTES_GITHUB_RELEASES_LATEST_API_URL,
  UUMINUTES_GITHUB_RELEASES_URL,
  UUMINUTES_INSTALLER_ASSET_NAME,
  UUMINUTES_INSTALLER_LATEST_DOWNLOAD_URL,
...
const log = createLogger('uuminutes/appUpdate');

const GITHUB_USER_AGENT = 'uuMinutes-Desktop';
...
function resolveInstallerDownloadUrl(release: GitHubLatestRelease): string {
  const asset = release.assets?.find(
    item => item.name === UUMINUTES_INSTALLER_ASSET_NAME
  );
  return (
    asset?.browser_download_url ?? UUMINUTES_INSTALLER_LATEST_DOWNLOAD_URL
  );
}
...
export function broadcastAppUpdateProgress(
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    if (!window.isDestroyed()) {
      window.webContents.send('uuminutes:app-update-progress', progress);
    }
  }
}
...
export function createAppUpdateProgressSender(
  ...
  return progress => {
    ...
    webContents.send('uuminutes:app-update-progress', progress);
  };
}
...
export async function checkForAppUpdate(
  ...
  if (!app.isPackaged) {
    return {
      ...
      releaseUrl: UUMINUTES_GITHUB_RELEASES_URL,
      downloadUrl: UUMINUTES_INSTALLER_LATEST_DOWNLOAD_URL,
      checkSkipped: true,
      errorMessage: null,
    };
  }
  ...
  try {
    ...
    return {
      ...
      latestVersion,
      updateAvailable,
      releaseUrl: release.html_url ?? UUMINUTES_GITHUB_RELEASES_URL,
