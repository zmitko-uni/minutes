...
const log = createLogger('uuminutes/bookmarks');

const BOOKMARKS_FILE_NAME = 'bookmarks.json';

type StoredBookmarks = {
  bookmarks: Array<UuMinutesBookmark>;
};
...
export async function addBookmark(
  ...
  if (existingIndex >= 0) {
    ...
    const updated: UuMinutesBookmark = {
      ...existing,
  ...
  const bookmark: UuMinutesBookmark = {
    id: randomUUID(),
